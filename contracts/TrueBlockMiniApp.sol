...
contract TrueBlockMiniApp is ReentrancyGuard, Ownable {
    ...
    function submitTruthCheck(
        ...
        bytes32 checkId = keccak256(abi.encodePacked(content, block.timestamp, msg.sender));
    ...
    function voteOnTruthCheck(bytes32 checkId, uint256 truthScore) external {
        ...
        uint256 newTotalScore = (check.truthScore * check.totalVotes) + truthScore;
        check.totalVotes++;
     ...
     */
    function getRecentChecks(uint256 limit) external view returns (bytes32[] memory) {
        uint256 startIndex = allChecks.length > limit ? allChecks.length - limit : 0;
     ...
     */
    function getCheckDetails(bytes32 checkId) external view returns (
        string memory content,
        uint256 timestamp,
        uint256 truthScore,
        uint256 totalVotes,
     ...
     */
    function getMiniAppStats() external view returns (
        uint256 totalChecks,
        uint256 verifiedChecks,
        uint256 totalUsers,
        uint256 totalVotes
    ) {
